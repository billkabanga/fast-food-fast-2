...
class Users:
    ...
    def get_admin(username):
        ...
        """
        query = "SELECT * FROM users WHERE username = '{}' AND role = 'admin'".format(
            username)
        new_db = Dbcontroller(app.config['DATABASE_URL'])
        return new_db.get_data(query)
    ...
    def validate_user_reg(username, email, contact, password, role):
        ...
        if not re.match(r"^[a-zA-Z]+$", username):
            return make_response(
                jsonify({
                    'message': 'Username should only have letters'
                }), 400)
        if not re.match(r"([\w\.-]+)@([\w\.-]+)(\.[\w\.-]+$)", email):
            return make_response(
                jsonify({
                    'message': 'wrong email entry'
                }), 400)
        if not re.match(r"^07[015789]\d{7}$", contact):
            return make_response(
                jsonify({
                    'message': 'Contact can only have 10 digits'
                }), 400)
        if len(password) < 8:
            return make_response(
                jsonify({
                    'message':
                    'Password should not be less that 8 characters'
                }), 400)
        if not role in roles:
            return make_response(
                jsonify({
                    'message': 'Role can only be admin or client'
                }), 400)
        return True
